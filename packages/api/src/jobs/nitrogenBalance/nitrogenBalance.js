...
const grabFarmIDsToRun = async (currDate) => {
  ...
  WHERE to_char(date(n.scheduled_at), 'YYYY-MM-DD') = ?`,
    [currDate],
  );
...
const runNitrogenBalance = async (farmIDJSON) => {
  ...
  const [inputN, outputN] = await Promise.all([
    inputNitrogenForFarm(farmID),
    outputNitrogenForFarm(farmID),
  ]);
...
`,
  ...
  */
  ...
  if (dataPoints.rows) {
    await Promise.all(
      dataPoints.rows.map(async (data) => {
        ...
        const totalNitrogenForFert =
          nitrogenCredits +
          (currentTotalNitrogen - currentNH3 - currentNH4) * mineralizationRate +
          currentNH4 +
          currentNH3;
    ...
    await Promise.all(
      dataPoints.rows.map(async (data) => {
        ...
        console.log(
          ...
          refuse,
          proteinContent,
          factorToConvertFromProteinToNitrogen,
          moistureFactor,
        );
        const totalNitrogenInCrop =
          quantityHarvested *
          (1 - refuse) *
          proteinContent *
          factorToConvertFromProteinToNitrogen *
...
const saveToDB = async (nitrogenBalanceByField) => {
  ...
  try {
    for (const key in nitrogenBalanceByField) {
      await nitrogenBalanceModel
        .query(trx)
        .insert({ location_id: key, nitrogen_value: nitrogenBalanceByField[key] })
        .returning('*');
    }
    await trx.commit();
  } catch (e) {
