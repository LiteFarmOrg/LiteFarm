...
export function* customLoginWithPasswordSaga({ payload: { showPasswordError, setAuth, ...user } }) {
  try {
    const screenSize = {
...
export function* customCreateUserSaga({ payload: { setAuth, user } }) {
  try {
    const { name, email, password, gender, birth_year } = user;
