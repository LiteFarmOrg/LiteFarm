...
export default function PurePlantingDate({
  ...
  const PLANTING_DATE = 'crop_management_plan.planting_date';
  ...
  const isCoverCrop = Boolean(persistedFormData.crop_management_plan.for_cover);
  const seedIsMain = !isInGround && persistedFormData?.crop_management_plan.is_seed;
  const seedlingIsMain = !isInGround && persistedFormData?.is_seed === false;
