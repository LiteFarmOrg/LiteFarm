export const up = async function (knex) {
  ...
  const fieldCrops = await knex('fieldCrop')
    .distinct('fieldCrop.field_crop_id', 'crop_variety.crop_variety_id')
    .join('location', 'location.location_id', 'fieldCrop.location_id')
    .join('crop_variety', 'crop_variety.crop_id', 'fieldCrop.crop_id')
    .where(knex.raw('location.farm_id = crop_variety.farm_id'));
  for (const fieldCrop of fieldCrops) {
    await knex('fieldCrop')
      .where({ field_crop_id: fieldCrop.field_crop_id })
      .update({ crop_variety_id: fieldCrop.crop_variety_id });
  }
};
