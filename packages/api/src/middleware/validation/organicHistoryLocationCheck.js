...
async function organicHistoryCheckOnPut(req, res, next) {
  const location = await locationModel.query().findById(req.params.location_id).withGraphJoined('[figure, field.[organic_history], garden.[organic_history], greenhouse.[organic_history]]');
  if (location && organicHistoryLocations.includes(location.figure.type)) {
    ...
    const organic_history_id = location[location.figure.type].organic_history.find((organicHistoryStatus) => {
      return moment(organicHistoryStatus.effective_date).utc().format('YYYY-MM-DD') === effective_date;
    })?.organic_history_id;
...
module.exports = { organicHistoryLocationCheckOnPost, organicHistoryCheckOnPut, organicHistoryCheckOnPost };
