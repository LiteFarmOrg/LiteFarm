...
describe('Field Tests', () => {
    ...
    describe('Get field tests', ()=>{
      ...
      beforeEach(async () =>{
        ...
        [managerFarm1] = await mocks.userFarmFactory({promisedUser:[newManager1], promisedFarm:[farm]},fakeUserFarm(2));
  ...
  describe('Delete field tests', ()=>{
    ...
    beforeEach(async () =>{
      ...
      [managerUserFarm] = await mocks.userFarmFactory({promisedUser: [managerUser], promisedFarm: [managerFarm]}, fakeUserFarm(2));

      [ownerUser] = await mocks.usersFactory();
      [ownerFarm] = await mocks.farmFactory();
      [ownerUserFarm] = await mocks.userFarmFactory({promisedUser: [ownerUser], promisedFarm: [ownerFarm]}, fakeUserFarm(1));

      [workerUser] = await mocks.usersFactory();
      [workerFarm] = await mocks.farmFactory();
      [workerUserFarm] = await mocks.userFarmFactory({promisedUser: [workerUser], promisedFarm: [workerFarm]}, fakeUserFarm(3));
    ...
    test('Manager should delete their field', async (done)=>{
      ...
      deleteRequest(fieldToBeDeleted, {user_id: managerUser.user_id, farm_id: managerFarm.farm_id}, async (err,res)=>{
        expect(res.status).toBe(200);
    ...
    test('should return 403 if a worker tries to delete a field', async (done)=>{
      ...
      deleteRequest(fieldToBeDeleted, {user_id: workerUser.user_id, farm_id: workerFarm.farm_id}, async (err,res)=>{
        expect(res.status).toBe(403);
        done();
      });
    })
    ...
    test('should return 403 if unauthorized user tries to delete a field', async (done)=>{
      ...
      deleteRequest(fieldToBeDeleted, {user_id: unAuthorizedUser2.user_id, farm_id: farmunAuthorizedUser2.farm_id}, async (err,res)=>{
        expect(res.status).toBe(403);
        done();
      });
    })
