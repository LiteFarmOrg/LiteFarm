...
// eslint-disable-next-line -- until migrated to TypeScript
import parseSensorCsv from '../../../../../../../libs/infra/csv/src/lib/sensorCSV';
...
export function useValidateBulkSensorData(onUpload, t) {
  ...
  const SENSOR_LATITUDE = t(
    'FARM_MAP.BULK_UPLOAD_SENSORS.SENSOR_FIELDS.LATITUDE'
  );
  const SENSOR_LONGITUDE = t(
    'FARM_MAP.BULK_UPLOAD_SENSORS.SENSOR_FIELDS.LONGITUDE'
  );
  const SENSOR_READING_TYPES = t(
    'FARM_MAP.BULK_UPLOAD_SENSORS.SENSOR_FIELDS.READING_TYPES'
  );

  // Optional Fields
  const SENSOR_EXTERNAL_ID = t(
    'FARM_MAP.BULK_UPLOAD_SENSORS.SENSOR_FIELDS.SENSOR_EXTERNAL_ID'
  );
  ...
  useEffect(() => {
    let validationErrorsResponseList =
    ...
    if (validationErrorsResponseList.length) {
      ...
      setUploadErrorMessage(
        t('FARM_MAP.BULK_UPLOAD_SENSORS.UPLOAD_ERROR_MESSAGE')
      );
    }
  }, [bulkSensorsUploadResponse?.validationErrors]);
  ...
  useEffect(() => {
    if (bulkSensorsUploadResponse?.errorSensors.length > 0) {
      setErrorCount(
        (curr) => curr + bulkSensorsUploadResponse.errorSensors.length
      );
      const translatedErrors = translateErrors(
  ...
  const translateErrors = (errors) => {
    return errors?.map((e) => {
      return {
        row: e.row,
        column: e.column,
        errorMessage: e.variables
          ? t(e.translation_key, e.variables)
          : t(e.translation_key),
        value: e?.value ?? '',
      };
    });
  ...
  const handleSelectedFile = async (e) => {
    ...
    try {
      ...
      if (fileExtension !== 'csv') {
        ...
        setUploadErrorMessage(
          t('FARM_MAP.BULK_UPLOAD_SENSORS.INVALID_FILE_TYPE')
        );
        ...
        setUploadErrorMessage(
          t('FARM_MAP.BULK_UPLOAD_SENSORS.EMPTY_FILE_UPLOAD_ERROR_MESSAGE')
        );
      }
  ...
  const onShowErrorClick = (errorCode) => {
    ...
    if (sheetErrors.length) {
      ...
      if (inputFile) {
        const downloadFileName = `${inputFile.name.replace(
          /.csv/,
          ''
        )}_errors.txt`;
