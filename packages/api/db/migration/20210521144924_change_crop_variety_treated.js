exports.up = async function(knex) {
    ...
    for (const { crop_variety_id, treated } of cropVarieties) {
      ...
      await knex('crop_variety').where({ crop_variety_id }).update({ treated: getTreatedEnum(treated) });
    }
};
...
exports.down = async function(knex) {
    ...
    for (const { crop_variety_id, treated } of cropVarieties) {
      ...
      await knex('crop_variety').where({ crop_variety_id }).update({ treated: getTreatedBool(treated) });
    }
};