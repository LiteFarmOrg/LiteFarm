...
export default function YearSelectorModal({ dismissModal, isOpen, initYear, selectYear}) {
  ...
  return (
    ...
    }} onClose={dismissModal} open={isOpen}>
      <DialogTitle classes={{
        root: styles.dialogTitle
      }}><Semibold style={{color: 'var(--teal700)', textAlign:'center'}}>{t('YEAR_SELECTOR.TITLE')}</Semibold></DialogTitle>
      <List>
        { years.map((year) => (
          <ListItem onClick={() => selectYear(year)} alignItems={'center'} style={{width: '227px'}} button>
