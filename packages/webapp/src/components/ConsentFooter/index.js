...
class ConsentFooter extends React.Component {
  ...
  render() {
    ...
    return hasConsent ? (
      <WithdrawConsent updateConsent={updateConsent} showModal={this.state.showModal}
                       setShowModal={setShowModal}/>
    ) : (
      <GiveConsent enableAgree={enableAgree} updateConsent={updateConsent} showModal={this.state.showModal}
                   setShowModal={setShowModal}/>
    )
  }
}
  ...
  return (
    <div className={styles.bottomContainer}>
      <button className='btn btn-primary' style={{ backgroundColor: '#4d4d4d69', borderColor: '#4d4d4d69' }}
              onClick={() => setShowModal(true)}>Disagree
      </button>
      ...
      />
      {enableAgree ?
        <button className='btn btn-primary' onClick={() => updateConsent({ 'consent': true })}>Agree</button> :
        <button style={{ backgroundColor: '#4d4d4d69', borderColor: '#4d4d4d69' }}
                className='btn btn-primary'>Agree</button>
      }
    </div>)
}
