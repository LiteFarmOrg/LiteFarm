...
export default function PureChooseFarmScreen({ onClick, farms = [], isSearchable, isOnBoarding, ...props }) {
  return <Layout buttonGroup={<Button onClick={onClick} fullLength>Let's get started</Button>}>
    ...
    <Link style={{ marginBottom: '24px', width: 'fit-content', fontSize: '16px', color: 'var(--iconActive)', lineHeight: '16px' }}
          to={'/add_farm'}>+ <Underlined>Add new farm</Underlined></Link>
    {isSearchable && <Input style={{ marginBottom: '16px' }} placeholder={'Search'} isSearchBar={true} {...props}/>}
    {farms.map((farm) => {
      return <ChooseFarmMenuItem style={{ marginBottom: '16px' }} farmName={farm.name} address={farm.address}
                                 color={farm.color} onClick={onClick}
