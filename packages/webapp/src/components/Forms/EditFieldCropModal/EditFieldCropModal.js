...
class EditFieldCropModal extends React.Component {
    ...
    componentDidMount() {
        ...
        fieldCrop.estimated_price = roundToTwoDecimal(fieldCrop.estimated_revenue / convertFromMetric(fieldCrop.estimated_production, estimated_unit, 'kg'));
  ...
  handleValuesReset(){
    ...
    let percentage = 0;
    if(is_by_bed){
      fieldCrop.estimated_yield = roundToTwoDecimal( convertFromMetric(fieldCrop.estimated_production, estimated_unit, 'kg') / bed_config.bed_num);
      fieldCrop.estimated_price = roundToTwoDecimal(fieldCrop.estimated_revenue / bed_config.bed_num / fieldCrop.estimated_yield);
    }
    else{
      fieldCrop.estimated_yield = roundToTwoDecimal(
        convertFromMetric(fieldCrop.estimated_production, estimated_unit, 'kg')
      ...
      <div>
        ...
        <Modal show={this.state.show} onHide={this.handleClose}>
          <Modal.Header closeButton>
            <Modal.Title>Edit - {this.props.cropBeingEdited.crop_common_name}, Variety: {this.props.cropBeingEdited.variety}</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <FormGroup>
              <h4 style={{textAlign: "center"}}>Edit how much of the field you are using ({this.state.area_unit_label}&sup2;)</h4>
              {
                isByArea && <div>
                    <FormGroup>
              ...
              <FormGroup controlId="start_date">
                <DateContainer  date={moment(this.state.fieldCrop.start_date)} onDateChange={this.onStartDateChange} placeholder="Choose a start date"/>
              </FormGroup>
              <FormGroup controlId="end_date">
                <DateContainer date={moment(this.state.fieldCrop.end_date)} onDateChange={this.onEndDateChange} placeholder="Choose a end date"/>
              </FormGroup>
