...
describe('LiteFarm end to end tests for tasks flow', () => {
  ...
  it('create and delete a custom irrigate task', () => {
    let waterUsageUnit;
    ...
    cy.get('[data-testid]')
      ...
      .then(($elem) => {
        const text = $elem.text();
        expect(text).to.equal(waterUsageUnit);
      });

    cy.get('[data-cy="delete-link"]').click();
    cy.get('[data-cy="taskReadOnly-complete"]').eq(1).click();
    cy.url().should('include', '/tasks');
    cy.visit('/tasks/3/read_only');
    cy.url().should('include', '/unknown_record');
  });
  ...
  it('Create a crop management plan with equal length row method plant task and container transplant task', () => {
    const date = new Date();
    date.setDate(date.getDate() + 1);
    const dueDate = getDateInputFormat(date);
    let lengthUnit,
