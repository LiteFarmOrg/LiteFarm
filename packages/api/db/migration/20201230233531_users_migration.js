  ...
  return knex('users')
    ...
    .then((users) => {
      ...
      const activeUsers = users
        .filter(({ user_id }) => /^\d+$/.test(user_id))
        .map(({ user_id }) => user_id);
      const isAllInvited = users.reduce(
        (obj, { user_id, status }) =>
          obj[user_id]
      ...
      const legacy = users
        .filter(
          ({ user_id }) => !allInvitedUserFarms.includes(user_id) && !activeUsers.includes(user_id),
        )
        .map(({ user_id }) => user_id);
      return Promise.all([
    ...
    .then(() => {
      ...
      return knex
        .raw(
          'SELECT user_id FROM users u WHERE user_id NOT IN (SELECT user_id FROM "userFarm" WHERE user_id = u.user_id)',
        )
        .then(({ rows: users }) => {
          const active = users
            .filter(({ user_id }) => /^\d+$/.test(user_id))
            .map(({ user_id }) => user_id);
          const legacy = users
            .filter(({ user_id }) => !active.includes(user_id))
            .map(({ user_id }) => user_id);
          return Promise.all([
