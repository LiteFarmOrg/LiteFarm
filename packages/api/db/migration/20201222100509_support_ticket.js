export const up = function (knex) {
  return Promise.all([
    knex.schema.createTable('supportTicket', function (table) {
      ...
      table
        .enu('support_type', ['Request information', 'Report a bug', 'Request a feature', 'Other'])
        .notNullable();
      table.text('message').notNullable();
      table.jsonb('attachments').defaultTo(JSON.stringify([]));
      table.enu('contact_method', ['email', 'whatsapp']).notNullable();
      table.string('email');
      table.string('whatsapp');
      table.enu('status', ['Open', 'Closed', 'In progress']).defaultTo('Open');
