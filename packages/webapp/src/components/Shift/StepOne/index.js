...
function PureStepOne({ onGoBack, onNext, workers, defaultWorker }) {
  ...
  let workerOptions = workers.map(({ first_name, last_name, user_id }) => ({label: `${first_name} ${last_name}`, value: user_id}));
  ...
  useEffect(() => {
    ...
    setWorker(defaultData.worker ?
      { value: defaultData.worker?.user_id, label: `${defaultData.worker.first_name} ${defaultData.worker.last_name}` } :
      { value: currentUser.user_id, label: `${currentUser.first_name} ${currentUser.last_name}` })
  }, [])

  const finishStepOne = () => {
    const selectedWorker = workers.find(({user_id}) => user_id === worker.value);
    const extendedSelectedTasks = taskTypes.filter(({task_id}) => selectedTasks.includes(task_id));
  ...
  return (
    <TitleLayout
      ...
      <>
        ...
        <Button type={'submit'} disabled={!selectedTasks.length || !worker  } fullLength onClick={finishStepOne}>
          {t('common:NEXT')}
        </Button>
      </>}
      onGoBack={onGoBack}
        ...
        taskTypes.map((type, i) => {
          ...
          return (
              <div className={styles.matrixItem} onClick={() => selectOrDeselectTask(type.task_id)}>
                <div className={clsx(styles.circleButton,  selected.includes(type.task_id) && styles.circleSelected)} id={type.task_id}>
                  <img data-test="task_type" src={buttonImg} alt="" />
                </div>
                <div className={styles.buttonName}><Text>{type.task_name}</Text></div>
              </div>
          );
        })
      }
