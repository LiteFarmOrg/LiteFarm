...
module.exports = (job) => {
  if(!job.data?.fail) {
    const { first_name, email, file, farm_id, farm_name, from_date, to_date, language_preference } = job.data;
    ...
    return sendEmail(emails.EXPORT_EMAIL, { first_name, farm_name, locale: language_preference }, email, {
      buttonLink,
    })
  } else {
    // Send failure email
