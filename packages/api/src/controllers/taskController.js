...
async function updateTaskWithCompletedData(
  ...
  if (typeOfTask === 'soil_amendment_task') {
    ...
    const { soil_amendment_task_product } = data;
    const unretireIds = [];
    soil_amendment_task_product.forEach((taskProduct) => {
      if (!taskProduct.id) {
      ...
      try {
        ...
        const result = await TaskModel.transaction(async (trx) => {
          ...
          const [task] = await TaskModel.query(trx)
            .withGraphFetched(
              `
          [locations.[location_defaults], managementPlans, taskType, soil_amendment_task, soil_amendment_task_product.[purpose_relationships], irrigation_task.[irrigation_type],scouting_task,
  ...
  async getTasksByFarmId(req, res) {
    ...
    try {
      ...
      const graphTasks = await TaskModel.query()
        .whereNotDeleted()
        .withGraphFetched(
          `[locations.[location_defaults], managementPlans, soil_amendment_task, soil_amendment_task_product.[purpose_relationships], field_work_task.[field_work_task_type], cleaning_task, pest_control_task, harvest_task.[harvest_use], plant_task, transplant_task, irrigation_task.[irrigation_type]]
        `,
        )
