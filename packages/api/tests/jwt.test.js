...
describe('JWT Tests', () => {
  ...
  async function deleteFarmRequestWithoutToken(data, user, callback) {
    return chai
      .request(server)
      .delete(`/farm/${data.farm_id}`)
      .set('farm_id', data.farm_id)
  ...
  async function postResetPasswordRequest(email, callback) {
    return chai
      .request(server)
  ...
  async function getValidateRequest(resetPasswordToken, user_id, callback) {
    return chai
      .request(server)
  ...
  async function putPasswordRequest(resetPasswordToken, user, callback) {
    return chai
      .request(server)
      .put(`/password_reset`)
  ...
  describe('Access jwt test', () => {
    test('should succeed on deleting a farm with valid token', async () => {
      ...
      await deleteFarmRequest(farm, newUser.user_id, async (_err, res) => {
    ...
    test('should fail on deleting a farm without valid token', async () => {
      ...
      await deleteFarmRequestWithoutToken(farm, newUser.user_id, async (_err, res) => {
  ...
  describe('Reset password jwt test', () => {
    ...
    test('Should send a valid token through email when reset_token_version === 0', async () => {
      ...
      await postResetPasswordRequest(newUser.email, async (_err, res) => {
        expect(res.status).toBe(200);
        const user = jsonwebtoken.verify(resetPasswordToken, process.env.JWT_RESET_SECRET);
        ...
        await getValidateRequest(resetPasswordToken, newUser.user_id, async (_err, res) => {
          expect(res.status).toBe(200);
    ...
    test('Should send a valid token through email when reset_token_version === 2', async () => {
      ...
      await postResetPasswordRequest(newUser.email, async (_err, _res) => {
    ...
    test('Should reject when reset_token_version === 3', async () => {
      ...
      await postResetPasswordRequest(newUser.email, async (_err, res) => {
        ...
        expect(created_at.getTime()).toBe(oldRow.created_at.getTime());
        await getValidateRequest(localResetPasswordToken, newUser.user_id, async (_err, res) => {
          expect(res.status).toBe(200);
          expect(res.body.isValid).toBe(true);
        });
      });
    ...
    test('Should reject when an invited user tries to get reset password token', async () => {
      ...
      await postResetPasswordRequest(invitedUser.email, async (_err, res) => {
        expect(res.status).toBe(400);
      });
    });
    ...
    test('Should reject when a pseudo user tries to get reset password token', async () => {
      ...
      await postResetPasswordRequest(pseudoUser.email, async (_err, res) => {
        expect(res.status).toBe(400);
      });
    });

    test('Should reject when an auth0 legacy user tries to get reset password token', async () => {
      const [_invitedUser] = await mocks.usersFactory({ ...mocks.fakeUser(), status_id: 3 });
      await postResetPasswordRequest(newUser.email, async (_err, res) => {
        expect(res.status).toBe(400);
      });
    });
  ...
  describe('Accept invitation jwt test', () => {
    ...
    async function postAcceptInvitationWithPasswordRequest(invitationToken, callback) {
      return chai
        .request(server)
    ...
    async function putAcceptInvitationWithGoogleAccountRequest(invitationToken, callback) {
    ...
    async function getRequest({ email }, callback) {
      return chai
        .request(server)
    ...
    beforeEach(async () => {
      ...
      checkGoogleJwt.mockImplementation(async (req, _res, next) => {
        req.auth = { ...googleUser };
        return next();
      });
      checkGoogleAccessToken.mockImplementation(async (req, _res, next) => {
        req.auth = { ...googleUser };
        return next();
      });
    ...
    test('Should create password and spotlight when user status is invited', async () => {
      ...
      await getRequest(user, async (_err, _res) => {
        const verified = jsonwebtoken.verify(invitationToken, tokenType.invite);
        expect(verified.user_id).toBe(user.user_id);
        await postAcceptInvitationWithPasswordRequest(invitationToken, async (_err, res) => {
    ...
    test('Should modify user_id and insert spotlight when login with google and user status is invited', async () => {
      ...
      await getRequest(user, async (_err, _res) => {
        ...
        await putAcceptInvitationWithGoogleAccountRequest(invitationToken, async (_err, res) => {
