exports.up = async function(knex) {
  const plantingPlantingManagementPlans = await knex('planting_management_plan')
    .join('crop_management_plan', 'crop_management_plan.management_plan_id', 'planting_management_plan.management_plan_id')
    .join('management_plan', 'management_plan.management_plan_id', 'planting_management_plan.management_plan_id')
    .where({ is_final_planting_management_plan: false, already_in_ground: false }).orWhere({
      needs_transplant: false,
      already_in_ground: false,
    });
  await knex('planting_management_plan')
    .whereIn('planting_management_plan_id', plantingPlantingManagementPlans.map(({ planting_management_plan_id }) => planting_management_plan_id))
