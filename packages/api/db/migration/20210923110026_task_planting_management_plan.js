exports.up = async function(knex) {
  ...
  for (const { management_plan_id, task_id } of managementTasks) {
    ...
    await knex('management_tasks').where({ management_plan_id, task_id }).update({ planting_management_plan_id });
  }
  await knex.schema.alterTable('management_tasks', t => {
...
exports.down = async function(knex) {
  ...
  for (const { planting_management_plan_id, task_id } of managementTasks) {
    const { management_plan_id } = await knex('planting_management_plan').where({ planting_management_plan_id }).first();
    await knex('management_tasks').where({ planting_management_plan_id, task_id }).update({ management_plan_id });
  }
  await knex.schema.alterTable('management_tasks', t => {
  ...
  await knex.raw('ALTER TABLE management_tasks RENAME CONSTRAINT management_tasks_pkey TO "activityCrops_pkey"');
