...
const getUnit = (system) => {
  return {
    [TEMPERATURE]:
      getUnitOptionMap()[ambientTemperature[system].defaultUnit].value,
    [SOIL_WATER_POTENTIAL]:
      getUnitOptionMap()[soilWaterPotential[system].defaultUnit].value,
    [SOIL_WATER_CONTENT]: '%',
  };
};
    ...
    for (let locationId of locationIds) {
      ...
      for (let type of readingTypes) {
        ...
        let typeReadings = data?.sensorReading[type].reduce((acc, cv) => {
          ...
          const currentValueDateTimeString = new Date(
            currentValueUnixTime * 1000
          ).toString();
          ...
          if (
            ...
            const hourlyTimezoneOffsetString =
              forwardUtcOffsetMinutes === 0
                ? '00'
                : Math.abs(forwardUtcOffsetMinutes).toString();
            const formattedCurrentValueDateTimeString = `${
              currentValueDateTimeString?.split(
                `:${hourlyTimezoneOffsetString}:00`
              )[0]
        ...
        if (type === TEMPERATURE) {
          // Call OpenWeather
          const centerPoint = findCenter(
            data?.sensorsPoints.map((s) => s?.point)
          );
          const params = {
          ...
          for (const weatherURL of OPEN_WEATHER_API_URL_FOR_SENSORS) {
            ...
            openWeatherPromiseList.push(
              call(axios.get, openWeatherUrl?.toString())
            );
          }
          const [
            openWeatherResponse,
