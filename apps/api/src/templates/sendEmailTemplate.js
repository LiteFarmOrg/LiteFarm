...
const emailTransporter = new EmailTemplates({
  ...
  send: true,
  preview:
    process.env.DEBUG === 'email-templates' || process.env.DEBUG === 'i18n:*',
  subjectPrefix:
    process.env.NODE_ENV === 'production' ? false : '[Development] ',
  transport: {
    host: 'smtp.gmail.com',
    port: 465,
    secure: true,
...
function sendEmail(
  ...
  try {
    ...
    if (
      attachments.length &&
      attachments[0] &&
      [emails.HELP_REQUEST_EMAIL.path, emails.MAP_EXPORT_EMAIL.path].includes(
        template_path.path
      )
    ) {
