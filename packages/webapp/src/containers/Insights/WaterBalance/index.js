  ...
  render() {
    ...
    if (waterBalanceData.length > 0) {
      ...
      renderedComponent = <div>
        <h4>Your scheduled water balance hasn't run yet, please check back in two days and ensure you have at least one soil analysis that records soil texture for a field to see water balance data for crops in that field. If the problem persists please contact LiteFarm.</h4>
      </div>
    } else {
      renderedComponent = (
        <div className={styles.newRunContainer}>
          <h4>Looks like this is your first time running this! For more info on what this does, please click the information button to see.</h4>
          <Button onClick={this.createWaterBalanceSchedule} variant="primary">Register Farm</Button>
        </div>
      );
    }
    ...
    return (
      <div>
        <div className={insightStyles.insightContainer}>
          <PageTitle title="Water Balance" backUrl="/Insights"
          rightIcon={true} rightIconTitle={"Water Balance"} rightIconBody={infoBoxBody}/>
          <div>
            {renderedComponent}
          </div>
        </div>
...
const infoBoxBody = <div>
  <p>The water balance tells you whether your crops have too little or too much water. It relies on weather data, and it is updated by your irrigation and soil texture data from your soil analysis logs.</p>
  <p>This feature has not been widely tested across farms with low surrounding weather station density so use with caution. We welcome feedback on how well it performs for your farm.</p>
</div>;
