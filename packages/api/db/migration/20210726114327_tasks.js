...
export const up = async function (knex) {
  ...
  const [soil] = await knex('taskType')
    .insert({ task_name: 'Soil Sample Results', task_translation_key: 'SOIL_RESULTS' })
    .returning('task_id');
  const [irrigation] = await knex('taskType')
    .insert({ task_name: 'Irrigation', task_translation_key: 'IRRIGATION' })
    .returning('task_id');
  await knex('taskType')
    .update({ task_name: 'Transport', task_translation_key: 'TRANSPORT' })
    .where({ task_id: 2 });
  await knex('taskType')
    .update({ task_name: 'Field Work', task_translation_key: 'FIELD_WORK' })
    .where({ task_id: 9 }); // Weeding -> Field Work
  await knex('taskType')
    .update({ task_name: 'Planting', task_translation_key: 'PLANTING' })
    .where({ task_id: 5 });
  await knex('taskType')
    .update({ task_name: 'Social', task_translation_key: 'SOCIAL' })
    .where({ task_id: 4 });
  ...
  await knex.schema.alterTable('task', (t) => {
    ...
    t.enu('status', ['PLANNED', 'LATE', 'FOR_REVIEW', 'COMPLETED', 'ABANDONED']).defaultTo(
      'PLANNED',
    );
  ...
  shiftTasks.map(
    async ({
      ...
      const [resultingTask] = await knex('task')
        .insert({ happiness, owner, assignee, type, due_date, duration, wage_at_moment })
        .returning('*');
      location_id !== null
        ? await knex('location_tasks').insert({ task_id: resultingTask.task_id, location_id })
        : await knex('management_tasks').insert({
            task_id: resultingTask.task_id,
            management_plan_id,
          });
      task_id < 11
        ? await knex(shiftTaskToTaskTable[task_id]).insert({ task_id: resultingTask.task_id })
        : task_id === 11
        ? await knex(shiftTaskToTaskTable[task_id]).insert({
            task_id: resultingTask.task_id,
            quantity_kg: 0,
            type: 'systemicSpray',
          })
...
export const down = async function (knex) {
  ...
  await knex('taskType')
    .update({ task_name: 'Delivery', task_translation_key: 'DELIVERY' })
    .where({ task_id: 2 });
  await knex('taskType')
    .update({ task_name: 'Weeding', task_translation_key: 'WEEDING' })
    .where({ task_id: 9 }); // Weeding -> Field Work
  await knex('taskType')
    .update({ task_name: 'Seeding', task_translation_key: 'SEEDING' })
    .where({ task_id: 5 });
  await knex('taskType')
    .update({ task_name: 'Social', task_translation_key: 'SOCIAL' })
    .where({ task_id: 4 });
  ...
  await knex.raw(formatAlterTableEnumSql('field_work_task', 'type', oldFieldWorkTypes));
