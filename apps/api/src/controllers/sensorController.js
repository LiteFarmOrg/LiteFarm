...
const sensorController = {
  async getSensorReadingTypes(req, res) {
    ...
    try {
      const sensorReadingTypesResponse =
        await SensorModel.getSensorReadingTypes(location_id);
  ...
  async getAllSensorReadingTypes(req, res) {
    ...
    try {
      const allSensorReadingTypesResponse =
        await SensorModel.getAllSensorReadingTypes(farm_id);
      const allReadingTypesObject = allSensorReadingTypesResponse.rows.reduce(
        (obj, item) => {
  ...
  async getBrandName(req, res) {
    try {
      ...
      const brand_name_response = await IntegratingPartnersModel.getBrandName(
        partner_id
      );
  ...
  async addSensors(req, res) {
    ...
    if (
      !isNaN(testTimerOverride) &&
      testTimerOverride >= 0 &&
      testTimerOverride <= 30000
    ) {
    ...
    try {
      const { access_token } =
        await IntegratingPartnersModel.getAccessAndRefreshTokens(
          'Ensemble Scientific'
        );

      const [{ language_preference }] = await baseController.getIndividual(
        UserModel,
        user_id
      );
      ...
      if (errors.length > 0) {
        return await sendResponse(
          () => {
            return res
              .status(400)
              .send({ error_type: 'validation_failure', errors });
          },
          async () => {
        ...
        if (esids.length > 0) {
          ...
          await registerOrganizationWebhook(
            farm_id,
          ...
          ({ success, already_owned, does_not_exist, occupied } =
            await bulkSensorClaim(
              access_token,
              organization.organization_uuid,
              esids
            ));
        ...
        const { registeredSensors, errorSensors } = data.reduce(
          (prev, curr, idx) => {
            if (
              success?.includes(curr.external_id) ||
              already_owned?.includes(curr.external_id)
            ) {
        ...
        const successSensors = sensorLocations.reduce((prev, curr, idx) => {
          if (curr.status === 'fulfilled') {
            ...
            errorSensors.push({
              row:
                data.findIndex((elem) => elem === registeredSensors[idx]) + 2,
              translation_key: sensorErrors.INTERNAL_ERROR,
              variables: {
                sensorId:
                  registeredSensors[idx].external_id ||
                  registeredSensors[idx].name,
              },
            });
          }
          return prev;
              ...
              return await sendSensorNotification(
                ...
                SensorNotificationTypes.SENSOR_BULK_UPLOAD_FAIL,
                {
                  error_download: {
                    errors: errorSensors,
                    file_name: 'sensor-upload-outcomes.txt',
                    success: successSensors.map(
                      (s) => s.sensor?.external_id || s.name
                    ), // Notification download needs an array of only ESIDs
                    error_type: 'claim',
                  },
                }
              );
  ...
  async updateSensorbyID(req, res) {
    try {
      ...
      await PointModel.query()
        .patch(sensorLocation)
        .where('figure_id', figureID[0].figure_id);
  ...
  async addReading(req, res) {
    ...
    try {
      ...
      const { rows: partnerSensorReadingTypes = [] } =
        await PartnerReadingTypeModel.getPartnerReadingTypeByPartnerId(
          partnerId
        );
      ...
      for (const sensor of Object.keys(req.body)) {
        ...
        let { rows: corresponding_sensor = [] } =
          await SensorModel.getLocationIdForSensorReadings(
            sensor,
            partnerId,
            farmId
          );
        if (!corresponding_sensor.length)
          return res.status(400).send('sensor id not found');
        corresponding_sensor = corresponding_sensor[0];
        for (const sensorInfo of sensorData) {
          const readingType = sensorInfo.parameter_category
            .toLowerCase()
            .replaceAll(' ', '_');
          const isPartnerReadingTypePresent = partnerSensorReadingTypes.some(
            (partnerSensorReadingType) =>
              partnerSensorReadingType.readable_value === readingType
          );
          ...
          const readingTypeStoredAsUnit =
            databaseUnit[readingType] ?? undefined;
          const isStoredAsUnit = unit == readingTypeStoredAsUnit;

          if (sensorInfo.values.length < sensorInfo.timestamps.length) {
            return res
              .status(400)
              .send('sensor values and timestamps are not in sync');
          }
      ...
      if (infoBody.length === 0) {
        ...
        const result = await knex
          .batchInsert('sensor_reading', infoBody, chunkSize)
          .returning('*');
        return res.status(200).json({
          sensorsAdded: result,
          readingTypeValidationError: readingTypeValidation.length
  ...
  async getReadingsByFarmId(req, res) {
    try {
      ...
      const result = await SensorReadingModel.getSensorReadingsInDaysByFarmId(
        farm_id
      );
  ...
  async getAllSensorReadingsByLocationIds(req, res) {
    try {
      ...
      const sensorsPoints = await SensorModel.getSensorLocationByLocationIds(
        locationIds
      );
      res
        .status(200)
        .send({
          ...
          sensorsPoints: sensorsPoints.rows,
        });
    } catch (error) {
      res.status(400).send(error);
    }
  ...
  async retireSensor(req, res) {
    ...
    try {
      ...
      const location = await baseController.getByFieldId(
        LocationModel,
        'location_id',
        location_id
      );
      ...
      const sensor = await baseController.getByFieldId(
        SensorModel,
        'location_id',
        location_id
      );
      ...
      if (partner_name != 'No Integrating Partner' && external_id != '') {
        const external_integrations_response =
          await FarmExternalIntegrationsModel.getOrganizationId(
            farm_id,
            partner_id
          );
        const org_id = external_integrations_response.organization_uuid;
        unclaimResponse = await unclaimSensor(
          org_id,
          external_id,
          access_token
        );
      ...
      const deleteResponse = await LocationModel.deleteLocation(
        trx,
        location_id,
        { user_id }
      );
