  ...
  describe('Get animal batches tests', () => {
    test('All users should get animal batches for their farm', async () => {
      ...
      for (const role of roles) {
        ...
        expect({ ...firstAnimalBatch, internal_identifier: 1 }).toMatchObject(res.body[0]);
        expect({ ...secondAnimalBatch, internal_identifier: 2 }).toMatchObject(res.body[1]);
      }
    });
  ...
  describe('Delete animal batch tests', () => {
    ...
    test('Must send animal batch ids', async () => {
      ...
      expect(res).toMatchObject({
        status: 400,
        error: {
          text: 'Must send animal batch ids',
        },
      });
    });
    ...
    test('Must send valid queries', async () => {
      ...
      expect(res1).toMatchObject({
        status: 400,
        error: {
          text: 'Must send valid animal batch ids',
        },
      });
