...
xdescribe('FieldCrop Tests', () => {
  ...
  describe('Get && delete && put fieldCrop', () => {
    ...
    beforeEach(async () => {
      ...
      [fieldCrop] = await mocks.management_planFactory({ promisedField: [field], promisedCropVariety: [cropVariety] });
    ...
    describe('Delete fieldCrop', function () {
      ...
      test('should delete a fieldCrop by owner', async (done) => {
        deleteRequest(`/field_crop/${fieldCrop.field_crop_id}`, {}, async (err, res) => {
          expect(res.status).toBe(200);
          const fieldCropRes = await managementPlanModel.query().context({showHidden: true}).where('field_crop_id', fieldCrop.field_crop_id);
    ...
    describe('Put fieldCrop', () => {
      test('should be able to edit the area_used field', async (done) => {
        ...
        putFieldCropRequest(fieldCrop, {}, async (err, res) => {
          expect(res.status).toBe(200);
          const newFieldCrop = await managementPlanModel.query().context({ showHidden: true }).where('crop_variety_id', cropVariety.crop_variety_id).first();
      ...
      test('Expired route should filter out non-expired fieldCrop', async (done) => {
        ...
        fieldCrop.area_used = field.figure.area.total_area * 0.1;
        fieldCrop.harvest_date = moment().add(10, 'd').toDate();
        await mocks.management_planFactory({}, fieldCrop);
      ...
      test('should change the harvest_date to a future date', async (done) => {
        ...
        putFieldCropRequest(fieldCrop, {}, async (err, res) => {
          expect(res.status).toBe(200);
          const newFieldCrop = await managementPlanModel.query().context({ showHidden: true }).where('crop_variety_id', cropVariety.crop_variety_id).first();
          expect(newFieldCrop.harvest_date.toDateString()).toBe(fieldCrop.harvest_date.toDateString());
          done();
        })
      });
      ...
      test('should change the harvest_date to a historical date', async (done) => {
        fieldCrop.area_used = field.figure.area.total_area * 0.1;
        fieldCrop.harvest_date = moment().subtract(10, 'd').toDate();
        putFieldCropRequest(fieldCrop, {}, async (err, res) => {
          expect(res.status).toBe(200);
          const newFieldCrop = await managementPlanModel.query().context({ showHidden: true }).where('crop_variety_id', cropVariety.crop_variety_id).first();
          expect(newFieldCrop.harvest_date.toDateString()).toBe(fieldCrop.harvest_date.toDateString());
      ...
      test('Expired route should not filter out non-expired fieldCrop', async (done) => {
        ...
        fieldCrop.area_used = field.figure.area.total_area * 0.1;
        fieldCrop.harvest_date = moment().subtract(10, 'd').toDate();
        await mocks.management_planFactory({ promisedCrop: [crop], promisedField: [field] }, fieldCrop);
        getRequest(`/field_crop/expired/farm/${farm.farm_id}`, {}, (err, res) => {
          expect(res.status).toBe(200);
  ...
  describe('Post fieldCrop', () => {
    ...
    test('Should post then get an expired crop', async (done) => {
      ...
      fieldCrop.harvest_date = moment().subtract(20, 'd').toDate();
      postFieldCropRequest(fieldCrop, {}, async (err, res) => {
        expect(res.status).toBe(201);
        const newFieldCrop = await managementPlanModel.query().context({ showHidden: true }).where('crop_variety_id', cropVariety.crop_variety_id).first();
        expect(newFieldCrop.field_id).toBe(field.field_id);
