...
class SaleController extends baseController {
  ...
  static addOrUpdateSale() {
    return async (req, res) => {
      ...
      try {
        ...
        const result = await baseController.upsertGraph(saleModel, req.body, { user_id }, trx);
        await trx.commit();
  ...
  static delSale() {
    return async (req, res) => {
      ...
      const trx = await transaction.start(Model.knex());
      try {
        const isDeleted = await baseController.delete(saleModel, req.params.sale_id, { user_id }, trx);
        await trx.commit();
        if (isDeleted) {
          res.sendStatus(200);
        } else {
