...
async function registerOrganizationWebhook(
  ...
  if (existingIntegration?.webhook_id) {
    ...
    const onResponse = async (response) => {
      await FarmExternalIntegrationsModel.updateWebhookId(
        farmId,
        response.data.id
      );
...
async function refreshTokens() {
  try {
    const { refresh_token } =
      await IntegratingPartners.getAccessAndRefreshTokens(
        'Ensemble Scientific'
      );
    const response = await axios.post(ensembleAPI + '/token/refresh/', {
...
async function authenticateToGetTokens() {
  try {
    ...
    const response = await axios.post(ensembleAPI + '/token/', {
      username,
      password,
    });
    await IntegratingPartners.patchAccessAndRefreshTokens(
...
async function unclaimSensor(org_id, external_id, access_token) {
  try {
    ...
    if (currentDeviceData?.owner_organization?.uuid !== org_id) {
      return {
        status: 200,
        data: { detail: 'Device not currently owned by this organization' },
      };
    }
    ...
    const response = await ensembleAPICall(
      access_token,
      unclaimAxiosObject,
      onError
    );
