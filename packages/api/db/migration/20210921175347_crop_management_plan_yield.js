exports.up = async function(knex) {
  ...
  const plantingManagementPlans = await knex('planting_management_plan').where('is_final_planting_management_plan', true);
  for (const planting_management_plan of plantingManagementPlans) {
    await knex('crop_management_plan').where({ management_plan_id: planting_management_plan.management_plan_id }).update({
...
exports.down = async function(knex) {
  ...
  for (const planting_management_plan of plantingManagementPlans) {
    await knex('planting_management_plan').where({
