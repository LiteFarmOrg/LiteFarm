...
exports.up = async function (knex) {
  ...
  await Promise.all(fertilizers.map(async  ({ fertilizer_type, fertilizer_id }) => {
    ...
    return Promise.all(soilTasks.filter(({ historic_product_id }) => historic_product_id === fertilizer_id).map(({ task_id }) => {
      return knex('soil_amendment_task').update({ product_id }).where({ task_id })
    }));
  }));
  await Promise.all(pesticide.map(async ({ pesticide_name, pesticide_id }) => {
    const insert = { name: pesticide_name, deleted: true, type: 'pest_control_task' };
    const [ { product_id }] = await knex('product').insert(insert).returning('*');
    return Promise.all(pestTasks.filter(({ historic_product_id }) => historic_product_id === pesticide_id).map(({ task_id }) => {
